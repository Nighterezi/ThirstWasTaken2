...
Everything else the pot shows is generated by `HangingPotModels`: the multipart blockstate, the frame
model that fills `#chain` with the chain texture of its version, and a flat water surface per fill
level and quality. The frame keeps `"render_type": "minecraft:cutout"` for NeoForge before 26.1; the
