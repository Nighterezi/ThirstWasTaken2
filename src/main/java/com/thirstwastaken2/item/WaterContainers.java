 ...
 * the copper canteen four, the iron flask six and the terracotta bowl one. Each loader turns a serving
 * into its own fluid unit, a bottle's 250 mB, and keeps the rules its APIs share on top of this.
 */
public final class WaterContainers {
    private WaterContainers() { }

    public static boolean handles(ItemStack stack) {
        return WaterskinItem.is(stack) || stack.is(ThirstItems.TERRACOTTA_BOWL)
                || stack.is(ThirstItems.TERRACOTTA_WATER_BOWL);
    }

    public static int capacity(ItemStack stack) {
        return WaterskinItem.is(stack) ? WaterskinItem.capacity(stack) : 1;
    }

    public static int servings(ItemStack stack) {
        if (WaterskinItem.is(stack)) return WaterskinItem.servings(stack);
