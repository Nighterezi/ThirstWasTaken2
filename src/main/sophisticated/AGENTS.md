# src/main/sophisticated — water quality through Sophisticated Core
...
both. Today it keeps water's grade through the **Tank upgrade**: a dirty bucket poured in comes back out
as dirty bottles, and sea water stays sea water. Without it the tank handed out plain water, which the
mod reads as `defaultPurity`, so any water went in and Clean water came out.
...
The gametests run without Sophisticated and prove the node still loads without it. With it,
[tools/agent/sophisticated-tank.jsonl](../../../tools/agent/sophisticated-tank.jsonl) builds four
backpacks from the templates in `tools/agent/sophisticated-tank-pack`, lets each tank run in the main
