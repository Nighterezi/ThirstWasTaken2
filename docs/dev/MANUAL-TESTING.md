...
- [ ] A client **with Jade and without** each integration's mod reaches the title screen and stays up,
      ...
      - `kaleidoscope_cookery` on `1.21.1-neoforge` and every Fabric node. Passed on
        `1.21.1-neoforge` on 2026-09-22 and on `1.21.1` and `26.3.x` on 2026-09-23, with Jade and
        without it
